...
function escapeLoneDollar(md: string): string {
  // Escape lone $ not part of $$ or already escaped, so remarkMath doesn't
  // misinterpret currency values like $10 as math delimiters.
  return md.replace(/(?<![\\$])\$(?!\$)/g, "\\$");
}

/**
 * Strip pi-internal HTML-like tags that break rehype-raw parsing.
 * Tags like <docId>, <read-files>, <modified-files> are generated by pi's
 * compaction system. When rehype-raw encounters them inside markdown list
 * items or backtick spans, it parses them as HTML elements, breaking the
 * DOM structure (e.g. causing narrow vertical text columns).
 * Session data is NOT modified — only the rendering input is sanitized.
 */
function stripInternalTags(md: string): string {
  return md
    .replace(/<docId[^>]*>[\s\S]*?<\/docId>/gi, "")
    .replace(/<docId[^>]*\/>/gi, "")
    .replace(/<docId\s*>/gi, "")
    .replace(/<\/docId>/gi, "")
...
export function MarkdownBody({ children, className, isStreaming, cwd, onOpenFile }: MarkdownBodyProps) {
  const normalizedMarkdown = useMemo(() => {
    ...
    const escaped = escapeLoneDollar(stripped);
    return normalizeDisplayMath(escaped);
  }, [children]);
